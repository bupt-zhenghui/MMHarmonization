...
def print_model_flops(model, inputs=None, image=None, pixel_pos=None, patch_pos=None, mask_r=None, mask=None, layers=[],
                      encode_only=False):  # retinextr
    ...
    output = model(inputs=inputs, image=image, pixel_pos=pixel_pos, patch_pos=patch_pos, mask_r=mask_r,
                   mask=mask)  # retinextr
    # output = model(inputs=inputs)
    total_flops = (sum(list_conv) + sum(list_deconv) + sum(list_linear) + sum(list_bn) + sum(list_relu) + sum(
        list_sigmoid) + sum(list_pooling))
