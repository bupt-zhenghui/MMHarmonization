...
class SNPatchDiscriminator(nn.Module):
    ...
    def __init__(self, opt):
        ...
        ndf = opt.ndf
        use_bias = True
        sequence = [nn.utils.spectral_norm(nn.Conv2d(opt.input_nc, ndf, kernel_size=kw, stride=2, padding=padw)),
...
class NLayerTFDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, no_antialias=True):
        ...
        dim = ndf * nf_mult
        self.transformer_enc = transformer.TransformerDecoders(dim, nhead=4, num_encoder_layers=4,
    ...
    def forward(self, input, pos, mask_r):
        ...
        tf_bg_output = self.transformer_enc(output.flatten(2).permute(2, 0, 1), tgt,
                                            src_key_padding_mask=fg_key_padding_mask, src_pos=pos, tgt_pos=query_embed)
        tf_all_output = self.transformer_enc(output.flatten(2).permute(2, 0, 1), tgt, src_key_padding_mask=None,
                                             src_pos=pos, tgt_pos=query_embed)
...
class ConvDiscriminator(nn.Module):
    ...
    def forward(self, input, fg_mask):
        ...
        fg_bg_conv = torch.matmul((bg_distribution - bg_distribution_mu).permute(0, 2, 1),
                                  (fg_distribution - fg_distribution_mu)) / C
...
class LLConvDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        #     nn.LeakyReLU(0.2, True)
        # ]
        sequence += [nn.Conv2d(ndf * nf_mult, ndf * nf_mult, kernel_size=3, stride=1,
    ...
    def forward(self, input, fg_mask):
        ...
        fg_bg_conv = torch.matmul((bg_pooling - bg_distribution_mu).permute(0, 2, 1),
                                  (fg_pooling - fg_distribution_mu)) / C
        fg_bg_r = fg_bg_conv.div(bg_distribution_std * fg_distribution_std + 1e-8)
        fg_bg_r = fg_bg_r.squeeze(-1)
        fg_bg_r = fg_bg_r.abs()
        return fg_bg_r
...
class PatchCoDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        self.transformer_enc = transformer.TransformerEncoders(dim, nhead=4, num_encoder_layers=4,
                                                               dim_feedforward=dim * 2, dropout=0.1)
        self.transformer_dec = transformer.TransformerDecoders(dim, nhead=1, num_encoder_layers=4,
                                                               dim_feedforward=dim * 2, dropout=0.1)
    ...
    def forward(self, inputs, fg_mask=None, postion_embedding=None):
        ...
        # fg_mask = 1-fg_mask
        input_mask_patch = util.extract_image_patches(torch.cat((inputs, fg_mask), dim=1),
                                                      ksizes=[self.ksizes, self.ksizes],
        ...
        bg_enc = self.transformer_enc(bg_transformer_input, src_pos=postion_embedding,
                                      src_key_padding_mask=fg_patch_mask.to(torch.bool))
        bg_dec = self.transformer_dec(bg_enc, tgt, src_pos=postion_embedding, tgt_pos=query_embed,
                                      src_key_padding_mask=fg_patch_mask.to(torch.bool), tgt_key_padding_mask=None)
        ...
        fg_enc = self.transformer_enc(fg_transformer_input, src_pos=postion_embedding,
                                      src_key_padding_mask=bg_patch_mask.to(torch.bool))
        fg_dec = self.transformer_dec(fg_enc, tgt, src_pos=postion_embedding, tgt_pos=query_embed,
                                      src_key_padding_mask=bg_patch_mask.to(torch.bool), tgt_key_padding_mask=None)
...
class GANLocalLoss(nn.Module):
    ...
    def __call__(self, prediction, fg_mask, image):
        ...
        luminance = torch.matmul(image_fg_patch_one, image_bg_patch_one.permute(0, 2, 1) + C1).div(
            image_fg_patch_one_sq + image_bg_patch_one_sq + C1)
